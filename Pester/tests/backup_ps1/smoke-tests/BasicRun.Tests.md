...
1. **Sandbox Isolation:** The test operates entirely within `Pester\resources\backup_ps1\smoke-tests\BasicRun\`.
2. **Environment Setup:** In the `BeforeAll` block, the test:
    ...
    - Creates `backup.ini` and `dir-list.conf` from templates.
    - Sets up a dummy `source/` folder with test files.
3. **Simulated Execution:** It runs the script using the `-SkipExecution` parameter. This allows the script to go through its entire logic (reading config, checking files, calculating paths, generating job files) but **prevents** it from actually executing Robocopy.
4. **Output Validation**: The test captures the script's console output (stdout and stderr).
    ...
    - **Additional Output Verification:** Regular Expressions are used to verify that expected log entries (INFO/DEBUG) and the "Skipping execution" message were generated.

### Dual-File Setup (test-backup.ini & backup.ini)
...
2. **Sandbox Isolation:** In the smoke test, `backup.ps1` is copied and renamed to `smoke-test.ps1`.
...
4. **Execution:** During setup, `test-backup.ini` is copied to `backup.ini`. This ensures that when `smoke-test.ps1` runs (or is invoked by the helper), it finds the configuration file.
...
If you add a new library file in `lib/` or a new template in `templates/`, the `BeforeAll` block in `BasicRun.Tests.ps1` will automatically copy the entire folders. However, if you add a **mandatory file check** in `backup.ps1`, you must:

- Add the file to the `BasicRun` resources or ensure it's simulated.
- Update `Pester\resources\backup_ps1\smoke-tests\BasicRun\test-backup.ini` to include the new configuration key.

### 2. Changing log messages or output

The test relies on regex matching of the console output. If you significantly change the wording of "INFO" or "WARNING" messages in the main script, you may need to update the `It` block in `BasicRun.Tests.ps1` to match the new strings.
...
If the test fails, check the `stdout.txt` file generated in the `Pester\resources\backup_ps1\smoke-tests\BasicRun\` directory. It contains the full console output of the last failed run.

### Minimal Resource Footprint

If you want to "empty" the `Pester\resources\backup_ps1\smoke-tests\BasicRun\` folder manually, you can delete everything **EXCEPT** for these mandatory files required for the test to start:
...
- **Files**: `smoke-test.ps1`, `backup.ini`, `dir-list.conf`, and `stdout.txt`.
