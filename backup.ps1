...
# Configuration object with default values. Using $script: scope so it's available for finally.
...
try {
  ...
  if (Test-Path "${iniFile}") {
    $script:Config = Read-Config -IniFile "${iniFile}"
  ...
  # Initialize the centralized logger with the final configuration.
  Initialize-Logger -Config $script:Config
  ...
  $FSobject = Get-ParentDir $script:Config.Logging.BACKUP_LOGFILE
  if (! ${FSobject}.Exists) {
    Write-EarlyMsg INFO "Creating logfile directory..."
    [void](New-Directory 'LOGFILE_DIR' "$($FSobject.Path)")
    Write-EarlyMsg INFO "Logfile directory created."
  }

  # Ensure the directory for the local trace log exists.
  if ($script:Config.Logging.TRACE_LOG_ENABLED -and ! [string]::IsNullOrWhiteSpace($script:Config.Logging.TRACE_LOG_TEMP_PATH)) {
    $TraceLogDir = Split-Path $script:Config.Logging.TRACE_LOG_TEMP_PATH -Parent
    if (! (Test-Path $TraceLogDir)) {
      New-Item -Path $TraceLogDir -ItemType Directory -Force | Out-Null
    }
  }

  # Flush early bootstrap messages to all enabled log files.
  Add-EmptyLineToLogfile # One empty line between the previous and this backup.

  foreach ($msg in $script:earlyMsgBuffer) {
    # Distribution to Primary, Error, and Trace logs is handled by Add-LogMessage.
    Add-LogMessage $msg.Severity $msg.Message
  }
  ...
  LogAndShowMessage INFO "Checking necessary directories and files..."
...
#>
  ...
  Test-NecessaryDirectory 'BACKUP_TEMPLATES_DIR' $script:Config.Directories.BACKUP_TEMPLATES_DIR
  Test-NecessaryFile 'DIRLIST_TEMPLATE' $script:Config.Files.DIRLIST_TEMPLATE
  Test-NecessaryFile 'JOB_TEMPLATE_INCR' $script:Config.Files.JOB_TEMPLATE_INCR
  Test-NecessaryFile 'JOB_TEMPLATE_FULL' $script:Config.Files.JOB_TEMPLATE_FULL
  Test-NecessaryFile 'JOB_TEMPLATE_PURGE' $script:Config.Files.JOB_TEMPLATE_PURGE
  Test-NecessaryFile 'JOB_TEMPLATE_ARCHIVE' $script:Config.Files.JOB_TEMPLATE_ARCHIVE
  Test-NecessaryFile 'JOB_TEMPLATE_GLOBAL_EXCLUSIONS' $script:Config.Files.JOB_TEMPLATE_GLOBAL_EXCLUSIONS
  Test-NecessaryFile 'JOB_TEMPLATE_LOGGING' $script:Config.Files.JOB_TEMPLATE_LOGGING

  # Note: Different cases for $BACKUP_BASE_DIR (some cannot be created)!
  $DirType = Get-SpecifiedBackupBaseDirType $script:Config.Directories.BACKUP_BASE_DIR
  Write-DebugMsg "BackupBaseDir type: ${DirType}"

  switch ("${DirType}") {
    "directory" {
      [void](New-Directory 'BACKUP_BASE_DIR' $script:Config.Directories.BACKUP_BASE_DIR)
    }
    { $_ -in "drive letter", "network share" } {
      Test-NecessaryDirectory 'BACKUP_BASE_DIR' $script:Config.Directories.BACKUP_BASE_DIR
    }
    "relative path" {
      # Relative to current working directory (NOT script dir anymore)
      [void](New-Directory 'BACKUP_BASE_DIR (relative path)' $script:Config.Directories.BACKUP_BASE_DIR)
    }
    "network computer" {
  ...
  [void](New-Directory 'BACKUP_USER_BASE_DIR' $script:Config.Directories.BACKUP_USER_BASE_DIR)
  [void](New-Directory 'BACKUP_DIR' $script:Config.Directories.BACKUP_DIR)
  [void](New-Directory 'BACKUP_JOB_DIR' $script:Config.Directories.BACKUP_JOB_DIR)

  # Make sure that robocopy has been found if only "robocopy" is defined in the ini file!
  $RobocopyExecutable = Get-ExecutablePath 'ROBOCOPY' $script:Config.Files.ROBOCOPY

  # Create the dir-list from the template if necessary.
  $IsDirlistCreated = New-FileFromTemplate 'BACKUP_DIRLIST' $script:Config.Files.BACKUP_DIRLIST $script:Config.Files.DIRLIST_TEMPLATE

  if ($IsDirlistCreated -and -not $NonInteractive) {
    Write-InfoMsg "Opening the dir-list in Editor and wait..."
    Notepad.exe $script:Config.Files.BACKUP_DIRLIST | Out-Null
  }

  LogAndShowMessage INFO "Necessary directories and files checked."
  ...
  $SelectedJobType = Get-UserSelectedJobType -DefaultJobType $script:Config.Jobs.DEFAULT_JOB_TYPE -MaxWaitingTimeS $script:Config.Jobs.JOB_TYPE_SELECTION_MAX_WAITING_TIME_S -NonInteractive:$NonInteractive

  switch ($SelectedJobType) {
    "Incremental" { $RobocopyJobTypeTemplate = $script:Config.Files.JOB_TEMPLATE_INCR }
    "Full" { $RobocopyJobTypeTemplate = $script:Config.Files.JOB_TEMPLATE_FULL }
    "Purge" { $RobocopyJobTypeTemplate = $script:Config.Files.JOB_TEMPLATE_PURGE }
    "Archive" { $RobocopyJobTypeTemplate = $script:Config.Files.JOB_TEMPLATE_ARCHIVE }
    "Cancel" { exit 0 }
    Default {
      # Illegal choice
  ...
  LogAndShowMessage INFO "Archiving previous jobs..."

  Export-PreviousJobsArchive $script:Config.Directories.BACKUP_JOB_DIR $script:Config.Jobs.JOB_FILE_NAME_SCHEME $script:Config.jobs.JOB_LOGFILE_NAME_SCHEME $script:Config.Archiving.ARCHIVE_NAME_SCHEME $script:Config.Archiving.MAX_ARCHIVES_COUNT

  LogAndShowMessage INFO "Previous jobs archived."
  ...
  LogAndShowMessage INFO "----- Creating job files -----".PadRight(70, "-")
  ...
  function Invoke-AddJobFile {
    ...
    Add-JobFile `
      $script:Config.Directories.BACKUP_JOB_DIR `
      $script:CurrentJobNum `
      $script:Config.Jobs.JOB_FILE_NAME_SCHEME `
      $script:Config.Jobs.JOB_LOGFILE_NAME_SCHEME `
  ...
  function _processDirectoryList {
    $DirListContent = Get-Content $script:Config.Files.BACKUP_DIRLIST
    ...
    ForEach ($line in $DirListContent) {
      ...
      - EOF
    #>
      $LineType = Get-DirlistLineType "${expanded}"
      ...
      switch -Wildcard ("${LineType}") {
        ...
        "invalid: *" {
          LogAndShowMessage WARNING "Invalid entry in dir-list: ${line}"
          $script:FinishPreviousJob = $true
          $Script:DirlistEntriesCount++
        }
        "ignore" {
      ...
      if ($script:ContinueCurrentJob) {
        if ($script:CurrentJobNum -eq 0) {
          $script:ContinueCurrentJob = $false
          LogAndShowMessage ERR "No folder/job defined for: ${line}"
        }
      }
      ...
      if ($script:StartNewJob) {
        ...
        if ("${Script:SourceDir}" -eq "") {
          LogAndShowMessage ERR "Parent directory not specified for: ${line}"
          Initialize-JobRelatedInfo
        }
        else {
          $script:TargetDir = Get-TargetDir $script:Config.Directories.BACKUP_DIR "${Script:SourceDir}"
  ...
  LogAndShowMessage INFO "$script:DirlistEntriesCount dir-list entries."
  LogAndShowMessage INFO "$script:JobsCreatedCount job file(s) created."
  ...
  $JobFiles = @(Get-ChildItem -Path (Join-Path $script:Config.Directories.BACKUP_JOB_DIR "*") -Include $script:Config.Jobs.JOB_FILE_NAME_SCHEME -File |
    Sort-Object { [int]([regex]::Match($_.Name, 'Job(\d+)\.RCJ').Groups[1].Value) })
  $JobfilesCount = $JobFiles.Count

  if ($JobfilesCount -eq 0) {
    LogAndShowMessage WARNING "No jobfiles created!"
  }
  else {
    LogAndShowMessage INFO "Running $JobfilesCount job(s)..."

    if ($SkipExecution) {
      LogAndShowMessage INFO "Skipping execution as requested (-SkipExecution)."
    }
    else {
      for ($i = 0; $i -lt $JobfilesCount; $i++) {
        ...
        if ($PSCmdlet.ShouldProcess("${UserDefinedJob}", "Run Robocopy job")) {
          & "${RobocopyExecutable}" `
            ...
            "/job:$($script:Config.Files.JOB_TEMPLATE_GLOBAL_EXCLUSIONS)" `
            "/job:$($script:Config.Files.JOB_TEMPLATE_LOGGING)" `
        ...
        LogAndShowRobocopyError "${JobName}" $RobocopyExitCode
      ...
      LogAndShowMessage INFO "$JobResultOkCount jobs finished successfully, $JobResultWarningCount with warnings, $JobResultErrorCount with errors."
  ...
  # Rotate and move the trace log to its final destination (Local-First approach).
  if ($script:Config.Logging.TRACE_LOG_ENABLED -and ! [string]::IsNullOrWhiteSpace($script:Config.Logging.TRACE_LOG_TEMP_PATH)) {
    if (Test-Path $script:Config.Logging.TRACE_LOG_TEMP_PATH) {
      if (! [string]::IsNullOrWhiteSpace($script:Config.Logging.TRACE_LOGFILE)) {
        $TargetTrace = $script:Config.Logging.TRACE_LOGFILE
        $BakTrace = "${TargetTrace}.bak"

        try {
          if (Test-Path $TargetTrace) {
            if (Test-Path $BakTrace) {
              Remove-Item $BakTrace -Force
            }
            Rename-Item $TargetTrace (Split-Path $BakTrace -Leaf) -Force
          }

          # Ensure the parent directory exists.
          $ParentTrace = Split-Path $TargetTrace -Parent
          if (! (Test-Path $ParentTrace)) {
            New-Item -Path $ParentTrace -ItemType Directory -Force | Out-Null
          }

          Move-Item $script:Config.Logging.TRACE_LOG_TEMP_PATH $TargetTrace -Force
          LogAndShowMessage INFO "Trace log moved to ${TargetTrace}"
        }
        catch {
          LogAndShowMessage WARNING "Failed to move trace log to destination. It is still available at: $($script:Config.Logging.TRACE_LOG_TEMP_PATH). Error: $_"
        }
      }
      else {
        LogAndShowMessage INFO "Trace log is available at: $($script:Config.Logging.TRACE_LOG_TEMP_PATH)"
      }
    }
  }
